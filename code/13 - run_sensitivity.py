...
for programme in range(N):
    ...
    counters = np.array(
        Parallel(n_jobs=-1)(
            delayed(ppi.run_ppi)(
                ...
                A=A, qm=qm, rl=rl,
                Imax=Imax, Imin=Imin, Bs=Bs_counter, B_dict=B_dict
            )
            for sample in tqdm(range(sample_size), desc="Running PPI", total=sample_size)
        )
    )
    ...
    dff.to_csv(home+'/data/sims/sensitivity.csv', index=False)
