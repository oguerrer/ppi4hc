...
groupo_names = ['Children', 'Youths', 'Young Adults', 'Middle-age Adults', 'Elderly']
weights_groups = dict(zip(groupo_names, [16/31, 8/31, 4/31, 2/31, 1/31]))
...
def run_par(I0, alpha, alpha_prime, beta, A, qm, rl, Imax, Imin, B, B_dict, solution):
    ...
    outputs = [ppi.run_ppi(I0, alpha, alpha_prime, beta, A=A, qm=qm, rl=rl, 
...
if opt_type == 'flat':
    best_fitness = np.average(df_base.values[:,-1], weights=None)
else:
    best_fitness = np.average(df_base.values[:,-1], weights=age_weights)
...
for step in range(10000):
    ...
    results = Parallel(n_jobs=popsize)(
        delayed(run_par)(I0, alpha, alpha_prime, beta, A, qm, rl, Imax, Imin, B, B_dict, solution) for solution in pop)    
    ...
    if fitness[best_idx] > best_fitness:
        ...
        if opt_type == 'flat':
            ...
            df_sol.to_csv(home+'/data/sims/optimal_B_weighted.csv', index=False)
